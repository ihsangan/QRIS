...
async function handleRequest(request) {
  const formData = await request.formData(); //new URL(request.url).searchParams
  ...
  if (o === 'html') {
    let qr = await fetch(`https://api.qrserver.com/v1/create-qr-code/?size=350x350&ecc=Q&qrzone=0&margin=0&&format=svg&data=${data}`)
  let content = `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"/><title>QRIS</title></head><body><img src="https://cdn.jsdelivr.net/gh/ihsangan/files/qris.svg" alt="QRIS logo" width="220" style="margin:27px 0"/><img src="https://cdn.jsdelivr.net/gh/ihsangan/files/gpn.svg" alt="GPN logo" width="50" style="float:right"/><br><center><img src="${qr.body}" alt="QRIS data" height="350"/><br><h3>Rp${price},00 ke ${name}</body></html>`;
