...
function getMerchInfo(d, p) {
  ...
  let res = `{"name":"${mn}","nmid":"${nmid}","price":"Rp${p.replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1.")},00"}`
  return res
};
async function handleRequest(request) {
  const formData = await request.formData()
  let d = formData.get('data');
  let p = formData.get('price').replace(/[.,]/g, '');
